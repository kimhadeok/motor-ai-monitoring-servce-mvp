...
def inject_global_styles() -> None:
    ...
    st.html(
        ...
        /* --- 이벤트 리스트: 좁은 화면은 카드로 (2026-08-13, 사용자 제보) ---
           **기준은 820px다.** 5열을 한 줄에 담으려면 열별 필요량 합계 579px + gap 64px =
           **643px**이 있어야 한다(config 실측). 그리드 폭은 대략 `뷰포트 - 129px`이라
           643px을 얻는 지점이 뷰포트 **772px**인데, 그 값을 그대로 쓰면 여유가 0이다 —
           실제로 770px에서 재보니 그리드 641px로 **2px 모자라** 세 열이 접혔다.
           모터명은 데이터에 따라 길이가 달라지므로 약 48px 여유를 두고 820px로 잡는다.

           종전 기준은 Streamlit이 컬럼을 접는 640px이었다. 필드마다 Streamlit 컬럼을 쓰던
           시절에는 그 값에 **묶여 있어야** 했지만(어긋나면 헤더는 가로인데 데이터는 세로가
           된다), 지금은 데이터 5필드가 우리 `.event-grid` 안이고 행의 컬럼은 둘(데이터부·
           버튼)뿐이라 우리가 자유롭게 정한다. 이 구간에서는 아래 `display: block`이 그 둘을
           직접 쌓으므로 Streamlit의 640px 동작과 겹쳐도 결과가 같다.
