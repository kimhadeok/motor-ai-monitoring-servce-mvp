...
# 최근 구간이 비어 모터 그래프(3시간 창)·모터 세부정보(6시간 창)와 카드 스파크라인이
# "데이터 없음"이 된다. 모터 존재 여부만 보던 종전 판정으로는 이 상황을 잡지 못했다.
# 가장 짧은 창(GRAPH_TREND_HOURS=3h)보다도 작게 잡아 창이 비기 전에 갱신되게 한다.
...
# --- 이벤트 리스트 레이아웃 (05 §3.3 / §4.4) ---
# **행 1건은 Streamlit 컬럼 2개다 — 데이터부 + 리포트 버튼** (2026-08-13 재구성).
#
# 종전에는 필드마다 컬럼을 하나씩 줘서 6개였다. 그런데 Streamlit이 각 컬럼 안 커스텀 HTML의
# 래퍼 높이를 실제보다 작게 잡아(실측: 내용 17.5px → 래퍼 1.5px), 640px 이하에서 컬럼이
# 세로로 쌓이자 칸끼리 겹쳐 찍혔다. 데스크톱에서는 행의 `align-items: stretch`가 그 오류를
# 가리고 있었을 뿐이다(05 §3.3).
#
# 이제 데이터 필드는 **마크다운 한 덩어리**로 넣고 CSS 그리드로 배치한다. 높이를 잘못 잴
# 래퍼가 하나뿐이라 그 문제가 생길 자리가 없고, 모바일 배치도 그리드 한 줄로 끝난다.
# 리포트 버튼만 별도 컬럼으로 남는다 — 다이얼로그를 여는 진짜 위젯이라 마크다운에 못 넣는다.
#
# **비율은 열별 "한 줄에 필요한 폭" 실측에서 나온다** (2026-08-13 재측정, 이벤트 10건 최대):
#   발생일시 65 · 모터명 163 · 상태변화 160 · 값변화 108 · 발생사유 83 = 합계 579px.
#
# 종전 비율(1.4 / 2.0 / 1.9 / 1.5 / 1.9)은 이 필요량과 어긋나 있었다 — **발생 사유가
# 83px만 있으면 되는데 상태 변화(160px)와 같은 1.9를 받고, 발생 일시도 65px에 1.4를
# 받았다.** 그래서 800px 뷰포트에서는 가용 폭(671px)이 필요량(579+gap 64=643px)보다
# 넓은데도 모터명과 상태 변화가 두 줄로 접혔다. 폭이 모자란 게 아니라 배분이 틀렸다.
#
# 아래 값은 필요량에 비례시키되, 글자 수가 데이터에 따라 늘 수 있는 두 열(모터명·발생
# 사유)에 여유를 더 준다. 발생 일시는 서식이 고정이라 여유가 필요 없다.
# 값 변화 열은 2026-08-07에 추가됐다 — 상태만으로는 임계를 아슬하게 넘었는지 크게
# 뛰었는지 알 수 없다.
#
# 컬럼 분할(데이터부 합계 : 버튼). 버튼은 데이터부의 약 11%다.
EVENT_ROW_SPLIT_WITH_MOTOR = (10.0, 1.2)
EVENT_ROW_SPLIT = (7.2, 0.85)  # 모터명(2.8)을 뺀 값 — 모터 상세(§4.4)가 쓴다
# 데이터부 내부 그리드. 헤더와 행이 같은 값을 공유해 열이 어긋날 수 없다.
EVENT_GRID_WITH_MOTOR = "1.1fr 2.8fr 2.8fr 1.8fr 1.5fr"
EVENT_GRID = "1.1fr 2.8fr 1.8fr 1.5fr"
#
# 모터 상세에서 모터명 열을 뺀 것은 2026-08-13 사용자 요청이다(2026-08-10 확정을 되돌림).
# 한 대의 이력만 보는 화면이라 모든 행에 같은 이름이 반복될 뿐이고, 모바일 카드에서는
# 카드마다 한 줄씩 그대로 낭비된다.
...
# 상세 그래프가 보여줄 최근 창(6시간). 모터 그래프 페이지는 3시간이다(`GRAPH_TREND_HOURS`) —
# 그쪽은 여러 대를 훑는 화면이라 최근 변화가, 상세는 한 대를 파고드는 화면이라 이력이 중요하다.
DETAIL_TREND_HOURS = LONG_TERM_TREND_HOURS
# Y축을 몇 단계로 나눌지(눈금 개수 = 단계 + 1). 세로로 두 배 벌어진 만큼 격자가 촘촘해야
# 값을 읽는다. **4 → 10단계 (2026-08-12 사용자 요청)** — 10단계면 상한이 등분되어 눈금이
...
# 라인차트가 보여줄 최근 창. **6 → 3시간 (2026-08-12 사용자 요청)** — 원본을 그대로 그리면서
# 6시간을 340px 폭에 밀어 넣으니 선이 굵은 띠처럼 뭉쳤다. 창을 줄이면 같은 폭에 절반의
# 시간이 들어와 변화가 벌어져 보이고, 점 수도 절반이 된다. 모터 세부정보는 6시간을 유지한다
# (`DETAIL_TREND_HOURS`) — 한 대만 보는 화면이라 넓은 이력이 더 쓸모 있다.
GRAPH_TREND_HOURS = 3
# X축 눈금·격자 간격(분). Vega 자동 선택(1시간)에 맡기지 않고 30분으로 고정한다 —
# 3시간 창에서 1시간 눈금은 격자가 두세 줄뿐이라 값이 언제 변했는지 짚기 어렵다.
GRAPH_X_TICK_MINUTES = 30
# 모터 행 머리의 상세 이동 버튼 — key 접두사(CSS로 높이를 줄인다)와 라벨 서식.
# 라벨에 모터명을 넣되 **구분자로 끊는다** (2026-08-12 사용자 요청) — "137호기 급수펌프 상세
# 보기"처럼 이어 붙이면 어디까지가 모터명인지 구분되지 않는다. 공백 2칸 + 하이픈 + 공백 2칸.
# 공백은 ` `(NBSP)다. 일반 공백은 HTML에서 연속 두 칸이 한 칸으로 합쳐져 구분이 약해진다.
GRAPH_DETAIL_BUTTON_PREFIX = "graphdetail-"
GRAPH_DETAIL_BUTTON_LABEL = "{motor_name}  -  상세 보기 ›"
...
# Y축 2단계 = 눈금 3개(0·중간·최상단). 130px에서 더 늘리면 Vega가 라벨을 솎아 최상단이
# 사라진다. 세부정보는 260px라 10단계까지 들어간다(`DETAIL_CHART_Y_DIVISIONS`).
GRAPH_CHART_Y_DIVISIONS = 2
...
# 관리자 화면에서 받는 설계 수명의 허용 범위(시간). 오타로 0이나 천문학적 값이 들어오면
# 경과율이 무한대가 되거나 0%로 굳는다.
MOTOR_LIFESPAN_HOURS_RANGE = (1_000, 200_000)

# **가동시간 산출 규칙 (2026-08-13 확정)** — 구동일자부터 지금까지를 하루 몇 시간 가동으로
# 볼 것인가. 24시간 연속 가동으로 잡는다: 이 서비스가 대상으로 하는 설비(송풍기·펌프·
# 압축기 등 공장 유틸리티)는 상시 가동이 기본이고, 실제 정지 이력을 받는 경로가 아직 없다.
#
# **가동률을 도입하려면 이 값만 바꾸면 된다.** 예를 들어 2교대(16시간)면 16으로 두면 되고,
# 경과율·잔여 수명·화면 표기가 모두 따라 바뀐다. 값을 낮추면 남은 수명이 늘어난다.
# 정지 이력을 반영하는 정식 계산은 실센서 연동 단계에서 다룬다(remaining_work #22).
...
STATUS_KOREAN_LABELS = {
    "NORMAL": "정상",
    # "주의" → "경고" (2026-08-12 사용자 요청). 차트 임계선 범례·관리자 임계값 화면은 이미
    # "경고"를 쓰고 있어 같은 상태를 두 이름으로 부르고 있었다. 상태 범례를 헤더에 상시
    # 노출하면서 한쪽으로 통일했다.
    "WARNING": "경고",
    "DANGER": "위험",
    "FAULT": "고장",
...
# 엑셀 일괄 등록 — **MVP는 버튼과 안내만 둔다** (2026-08-12 사용자 요청).
# 모터 200대를 화면 폼으로 한 대씩 넣는 것은 실제 도입에서 성립하지 않으므로 고객에게
# "이렇게 등록하게 됩니다"를 보여주는 자리다. 파일 생성·파싱은 정식 서비스에서 붙인다.
#
# **비활성 버튼으로 두지 않는다.** 눌러도 아무 반응이 없으면 기능 소개가 아니라 고장난
# 화면으로 읽힌다. 누르면 무엇을 할 수 있는지 안내가 뜬다.
#
# 화면 문구에 "시연"이라는 말을 쓰지 않는 것은 임계값 탭 안내와 같은 이유다 — 캡처 한 장이
# 돌면 그 화면은 계속 "시연용"으로 읽힌다.
ADMIN_BULK_BUTTON_PREFIX = "admin-bulk-"
ADMIN_BULK_SAMPLE_LABEL = "엑셀 샘플 다운로드"
ADMIN_BULK_UPLOAD_LABEL = "업로드 등록"
ADMIN_BULK_NOTICE_TAIL = "정식 서비스에서 제공됩니다. 지금은 아래 양식으로 한 건씩 등록합니다."
# 대상별 안내 문구. `sample`은 양식의 열 구성을, `upload`는 올린 뒤 무슨 일이 일어나는지를
# 말한다 — 둘 다 "무엇을 준비해야 하나"에 답해야 소개가 된다.
ADMIN_BULK_NOTICES = {
    "motor": {
        "sample": (
            "모터 등록용 엑셀 양식을 내려받아 여러 대를 한 번에 작성할 수 있습니다. "
            "열 구성: 모터 ID · 모터명 · 설치 위치 · 모델명 · 시리얼 번호 · 수집 주기(초)."
        ),
        "upload": (
            "작성한 엑셀을 올리면 형식과 중복 ID를 검사한 뒤 일괄 등록합니다. "
            "문제가 있는 행은 행 번호와 사유를 함께 알려 줍니다."
        ),
    },
    "threshold": {
        "sample": (
            "지표 임계값 등록용 엑셀 양식을 내려받아 모터별로 한 번에 작성할 수 있습니다. "
            "열 구성: 모터 ID · 지표(온도/진동/전류/소음) · 정상 · 경고 · 위험 · 고장."
        ),
        "upload": (
            "작성한 엑셀을 올리면 `정상 < 경고 < 위험 < 고장` 순서를 검사한 뒤 일괄 반영합니다. "
            "바뀐 기준은 개별 편집과 똑같이 다음 수집분부터 적용됩니다."
        ),
    },
}
...
#
# 그런데 그 촘촘함은 **오래된 구간에서는 화면에 보이지 않는다.** 화면이 보는 창은 길어야
# 6시간이고(그래프 3h · 세부정보 6h · 카드 스파크라인 6h), 카드 스파크라인은 그마저
# 15분 버킷 평균으로 압축한다(두 그래프 화면은 2026-08-12부터 원본 그대로 그린다).
