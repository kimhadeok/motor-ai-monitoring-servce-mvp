...
#
# 비율의 출처는 2026-08-07 실측 콘텐츠 필요량이다 (1440 뷰포트, 전체 1270px):
#   발생일시 110 · 모터명 204 · 상태변화 161 · 값변화 107 · 발생사유 82.
# 값 변화 열은 2026-08-07에 추가됐다 — 상태만으로는 임계를 아슬하게 넘었는지 크게 뛰었는지
# 알 수 없다. 아래 네 상수가 모두 이 한 벌에서 파생하므로 두 화면의 열이 같은 폭으로 읽힌다.
#
# 컬럼 분할(데이터부 합계 : 버튼 1.0)
EVENT_ROW_SPLIT_WITH_MOTOR = (8.7, 1.0)  # 1.4+2.0+1.9+1.5+1.9 : 1.0
EVENT_ROW_SPLIT = (6.7, 1.0)  # 모터명(2.0)을 뺀 값 — 모터 상세(§4.4)가 쓴다
# 데이터부 내부 그리드. 헤더와 행이 같은 값을 공유해 열이 어긋날 수 없다.
EVENT_GRID_WITH_MOTOR = "1.4fr 2fr 1.9fr 1.5fr 1.9fr"
EVENT_GRID = "1.4fr 1.9fr 1.5fr 1.9fr"
#
