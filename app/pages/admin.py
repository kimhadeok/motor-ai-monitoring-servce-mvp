...
st.subheader("관리자")
# 화면 상단 문구를 줄였다 (2026-08-12 사용자 요청): 종전에는 여기에 "로그인한 회사(...)의
# 정보만 보입니다"가 붙고, 아래에 재시드로 데이터가 초기화된다는 ⚠️ 경고 박스가 따로 있었다.
# 회사 격리는 헤더가 이미 회사명을 보여주고 있어 중복이었고, 경고 박스는 화면 맨 위 한 칸을
# 통째로 쓰면서 정작 관리할 표를 아래로 밀어냈다.
st.caption(
    "고객 회사 · 담당자 · 모터 · 지표 임계값을 관리합니다. "
    "계측 데이터는 여기서 다루지 않습니다."
)
...
with _company_tab:
    ...
    if _company is None:
        ...
        #
        # **캡션은 한 줄이다 (2026-08-12 고객 의견 2차 2-(5)).** 관리자를 누르면 이 탭이
        # 먼저 열리는데, 비활성 입력 2칸에 "왜 아무것도 못 하는지"를 3줄로 해명하고 있어
        # 설명이 내용보다 길었다. 관리 화면의 첫인상으로는 손해다. 아래 두 가지는 화면에서
        # 빼고 여기와 스펙(`05 §6.3`)에만 둔다:
        #   - 회사 추가를 두지 않는 이유: 자기 회사만 보이는 구조라 추가해도 화면에 안 나온다.
        #   - 회사 삭제를 두지 않는 이유: 자기 회사를 지우면 로그인이 성립하지 않는다.
        st.caption("회사 정보는 조회만 합니다 — 회사명 변경은 계약 처리로 진행합니다.")
...
with _threshold_tab:
    if not _motors:
        ...
        # **두 안내를 한 박스로 합쳤다** (2026-08-12 사용자 요청). 종전에는 "언제부터
        # 적용되는가"(🕒)와 "그래도 알림은 안 간다"(🔔)가 색이 다른 박스 둘로 나뉘어,
        # 표를 편집하기 전에 읽어야 할 것이 화면 두 칸을 차지했다. 같은 질문("바꾸면
        # 무슨 일이 일어나는가")에 대한 답이라 한 덩어리로 읽히는 편이 낫다.
        #
        ...
        # 단계에서 붙입니다"라고 말해, 같은 사실을 두 곳에서 다르게 말했다 — 듣는 쪽에는
