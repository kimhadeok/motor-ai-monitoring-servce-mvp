...
|---|---|---|
| 콜드 (배포 환경, 첫 기동) | **2.67초** | 스키마 0.03s · 시드 2.08s · RAG 확인 0.55s |
| 웜 (로컬 재기동) | 약 0.5초 | 데이터가 있고 신선해 시드 생략 |

> **시드는 48시간을 걷되 저장은 필요한 것만 한다 (2026-08-11).** 화면이 보는 창은 최근 6시간뿐이고 그래프는 15분 버킷 평균으로 압축한다. 그래서 최근 8시간은 전량, 전이 시각 행과 그 직전 행은 반드시, 나머지는 15분 간격으로 저장한다. **288,210행 3.80초 → 81,858행 1.95초**(전이 80건·알림 24건 불변). 자세한 근거는 `02_architecture.md §6.1`.
...
| 부팅 시드 소요 | `부트스트랩 완료 \| … N초` | 로컬 2.67초 대비 비교. 직전 배포본은 23.28초였고 시드 저장량을 72% 줄였으므로 **얼마나 줄었는지 여기서 판정한다** |
