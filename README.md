...
- **모터 수명 임박 알림 · 수명별 그룹핑** — 수명 관리 자체는 2026-08-13에 구현했다(아래 본문 참고). 남은 것은 ① 남은 수명 30·20·10일 도달 시 담당자 문자 발송(`03_state_event_logic.md` §4.5) ② 모터 현황의 수명 구간별 그룹핑(`05_ui_screens.md` §3-B)이다. 둘 다 스펙은 확정됐고, **알림은 실시간 전이 감지와 같은 제약을 받는다** — 수명은 아무 일도 없어도 시간이 지나면 임계에 닿으므로 상시 실행 경로가 필요하다. 상세는 `remaining_work` #22
