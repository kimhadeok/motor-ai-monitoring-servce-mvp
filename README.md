...
**모터 수명 관리 (2026-08-13)**: 모터마다 설계 수명(시간)과 구동일자를 두고, 모터 상세의 기본 정보에서 **남은 수명(`3년 2개월 14일 5시간` 형태)과 수명 경과율**을 봅니다. 관리자 페이지에서 두 값을 등록·수정합니다. 가동시간은 **하루 24시간 연속 가동**으로 환산하며(`config.MOTOR_DAILY_OPERATING_HOURS`), 가동률을 도입하려면 이 상수만 바꾸면 화면 표기가 모두 따라 바뀝니다. 구동일자는 모니터링 서비스 **등록일자와 다릅니다** — 설비는 서비스 도입 전부터 돌고 있으므로 구동일자가 앞섭니다. 경과율은 소수점 2자리에 **버림**이라, 100%는 실제로 수명을 다 썼을 때만 나옵니다. 자세한 내용은 `05_ui_screens.md` §4.1.
...
- **모터 수명 임박 알림 · 수명별 그룹핑** (2026-08-13 확정) — 수명 관리 자체(상세 표시 · 관리자 입력 · 리포트 반영)는 구현했고, 남은 둘만 정식 서비스로 미룬다: ① 남은 수명 30·20·10일 도달 시 담당자 문자 발송(`03_state_event_logic.md` §4.5) ② 모터 현황의 수명 구간별 그룹핑(`05_ui_screens.md` §3-B). **알림은 위 실시간 전이 감지와 같은 제약**이라 함께 풀어야 한다 — 수명은 아무 일도 없어도 시간이 지나면 임계에 닿으므로 상시 실행 경로가 필요하다. 그룹핑은 알림과 구간 경계를 공유해 짝으로 미룬다. **MVP에서도 수명은 상세·리포트에서 확인된다**
