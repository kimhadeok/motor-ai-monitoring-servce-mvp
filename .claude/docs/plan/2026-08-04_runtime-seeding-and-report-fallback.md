...
| 항목 | 이전 | 현재 |
|---|---|---|
| 텔레메트리 | 6시간 | 48시간 (63,366행, 0.67초) |
| 이벤트 분포 | MTR-001 단독 | 두 회사 4대 (MTR-001/003/004/006) |
| FAULT | 없음 | 있음 (MTR-004) |
| 급변(단계 스킵) | 없음 | 1건 |
| 상태 로그 | 87건 (플래핑) | 25건 |
| 알림 | DANGER 전건 16건 | 3건 (쿨다운으로 1건 차단) |
| 임베딩 차원 | 384 (MiniLM) | 1536 (OpenAI) |

### 계획 대비 정정 사항

**쿨다운 적용 위치.** 점검 시 "상태 로그 플래핑"을 쿨다운 미적용 문제로 지목했으나, `03_state_event_logic.md` §5 원문은 *"쿨다운 중 신규 AI 에이전트 재가동 및 알림 발송만 차단. 로그 자체는 계속 적재"* 이다. 로그 적재는 문서대로 맞았고 실제 누락은 **알림 발행 쪽**이었다. 따라서 쿨다운은 `_seed_notifications()`에만 적용했고, 플래핑은 별개의 데이터 품질 문제로 보아 히스테리시스(`TRANSITION_CONFIRM_SAMPLES=3`)로 처리했다.

### 4단계 착수 시 확인할 것

**부팅 시간이 목표를 초과한다.** 단계별 실측 합계가 시드 0.67s + RAG 인제스트 2.55s + 리포트 HTML 0.54s ≈ **3.8초**로 §6.1의 3초 목표를 넘는다. RAG 인제스트가 주원인인데, 순수 임베딩 API 호출만 쟀을 때는 0.60초였으므로 차이는 Chroma 클라이언트 초기화·컬렉션 생성 오버헤드로 보인다. 실제 부팅에서 다시 측정한 뒤 필요하면 조정한다(예: 컬렉션이 이미 채워져 있으면 인제스트 생략).
