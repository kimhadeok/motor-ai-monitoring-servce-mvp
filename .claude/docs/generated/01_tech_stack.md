...
| Altair (Vega-Lite) | 시계열 차트 — `st.altair_chart` (온도/진동/전류/소음). 라인 + 상태 임계선. 두 화면 모두 수집 원본을 그대로 그리므로(2026-08-12) 포인트 마커는 끈다 — 점이 수천 개라 선이 덮인다. 모터 그래프 페이지(`05 §3-A`)와 모터 상세(`05 §4.1-A`)가 `app/ui/charts.py`를 **공유**한다 — 축 구성·임계선·Y범위 규칙이 한 벌이어야 두 화면을 같은 눈으로 읽을 수 있다. 창 길이·높이·Y단계·X눈금 간격은 화면별 인자다. *(Plotly는 의존성에 있으나 현재 미사용 — 그래프는 Altair로 구현)* |
