...
**인제스트 시점 (2026-08-07 확정).** 원본이 시간에 무관한 정적 텍스트이므로 **부팅 시 인제스트하지 않는다.** `scripts/build_knowledge.py`로 수동 1회 실행하고 산출물 `data/chroma/`(46청크, 1.3MB)를 커밋한다. Community Cloud가 초기화하는 것은 런타임에 쓴 파일이고 커밋된 파일은 체크아웃으로 들어오므로 배포본이 그대로 사용한다.
