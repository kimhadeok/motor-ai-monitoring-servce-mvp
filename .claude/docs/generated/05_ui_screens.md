...
- 회사명은 상단 캡션, 나머지 4개는 `st.metric` 4열로 배치한다. "주의 이상 모터 수"는 합계를 값으로, `WARNING n · DANGER n · FAULT n` 내역을 delta 영역에 표기한다(증감이 아니므로 `delta_color="off"`).
...
- **다이얼로그 상태 유지**: 확인 중인 지표를 세션 상태에 담아둔다. 버튼 클릭 여부로만 다이얼로그를 열면 그 안의 체크박스를 누르는 순간 rerun이 일어나 다이얼로그가 닫히고 확인 절차를 마칠 수 없다. 확인·취소 시 세션 키를 지운다.
