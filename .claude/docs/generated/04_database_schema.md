...
> - **NULL을 허용한다.** 관리자 화면에서 두 값을 입력할 수 있게 됐지만(`05 §6.3`, 2026-08-13 구현), 그 이전에 등록된 모터와 마이그레이션으로 컬럼만 붙은 기존 행은 값이 비어 있다. 값을 읽는 쪽은 여전히 NULL을 전제해야 한다 — `services/motors.motor_lifespan_info()`가 둘 중 하나라도 없으면 `None`을 반환하고 화면은 `-`로 적는다.
...
> - **읽는 쪽 (2026-08-13 구현)**: `services/motors.motor_lifespan_info()`가 경과율·잔여 수명의 **유일한 출처**이고, 모터 상세 §4.1이 구동일자·모터 수명·남은 수명·경과율 4항목을 표시한다(`05 §4.1`). 가동시간은 `config.MOTOR_DAILY_OPERATING_HOURS`(하루 24시간 연속)로 환산한다.
> - **아직 없는 것**: 수명 임박 알림(`03 §4.5`)과 모터 현황 수명별 그룹핑(`05 §3-B`). 둘 다 스펙은 확정됐고 구현만 남았다(`remaining_work` #22).
...
6. **motors.lifespan_hours / operation_started_at 추가 (§3.3, 2026-08-13 사용자 요청)**: 모터 수명 관리용. 설계 수명(시간)과 실제 가동 시작일을 한 쌍으로 둔다. 같은 날 **가동시간 산출 규칙 확정(하루 24시간) · 경과율·잔여 수명 계산 · 모터 상세 표시 · 관리자 입력까지 구현**했다. 남은 것은 수명 임박 알림(`03 §4.5`)과 수명별 그룹핑(`05 §3-B`)이며 둘 다 스펙만 확정된 상태다(`remaining_work` #22).
