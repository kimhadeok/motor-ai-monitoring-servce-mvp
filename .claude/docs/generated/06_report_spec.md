...
| **의심 고장 모드 (2026-08-07 추가)** | 참조 지식(`01_tech_stack.md` §2.3.1)에서 해당 지표로 **결정적 조회**. 항목별로 고장명(한/영), 여유 시간대, 진단 근거, 교체 대상 부품, 출처 문서·페이지를 표시한다. 지식이 없으면 섹션 자체가 사라진다 |
...
1. 헤더/메타 정보
2. 센서 측정 데이터 (4개 지표)
3. AI 진단 및 근본 원인 분석 (신뢰도 게이지 제외, 텍스트 + **의심 고장 모드**)
